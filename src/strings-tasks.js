...
function removeFirstOccurrences(str, value) {
  const val = str.indexOf(value);
  if (val !== -1) {
    return str.slice(0, val) + str.slice(val + value.length);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const val = str.lastIndexOf(value);
  if (val !== -1) {
    return str.slice(0, val) + str.slice(val + value.length);
  }
  return str;
...
function isPalindrome(str) {
  const symbols = /[.,/?!^&*;:{}=\-_`~()]/g;
  const str2 = str.toLowerCase().replaceAll(' ', '').replaceAll(symbols, '');
  const reversed = str2.split('').reverse().join('');
...
function reverseWords(str) {
  ...
  const result = splitted.map((val) => val.split('').reverse().join(''));
  return result.join(' ');
}
...
function extractNameFromTemplate(value) {
  ...
  const lastName = fullName[2].replace('!', '');
...
function encodeToRot13(str) {
  ...
  keys.forEach((val1, val2) => {
    obj[val1] = values[val2];
  });

  let result = '';

  const splittedStr = str.split('');
  splittedStr.forEach((symbol) => {
    if (Object.hasOwn(obj, symbol)) {
      result += symbol.replace(symbol, obj[symbol]);
    } else {
      result += symbol;
    }
  });
...
function getCardId(value) {
  ...
  return cards.findIndex((val) => val === value);
}
